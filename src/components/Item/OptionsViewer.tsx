...
export const OptionsViewer: React.FC<OptionViewerProps> = (props) => {
    ...
    const { options } = panelId === -1 ? panel : panel.list[panelId];
    ...
    const handleSaveName = useCallback(
        (event: KeyboardEvent<HTMLInputElement>) => {
            if (event.which === 13) {
                ...
                actions.savePanels(name, panel.list.map(x => x.options));
                setSaveName('');
            }
        },
        [panel.list, actions]
    ...
    const handleSaveOptions = useCallback(() => {
        actions.savePanels(saveName, panel.list.map(x => x.options));
        setSaveName('');
    },
        [saveName, panel.list, actions]
    );
