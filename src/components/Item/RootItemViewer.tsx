...
export const RootItemViewer: React.FC = () => {
    ...
    const { items, itemList, panel, panelNames, changeList, changes } = useSelector(
    ...
    return (
        <div style={{display: 'flex', width: '100%'}}>
            <div>
                Changes
                {changeList.map(x => <div style={{margin: '5px 0'}} key={changes[x].changeId}>{changes[x].type} {changes[x].response}<br />{(changes[x] as ItemChange).itemId}</div>)}
            </div>
            <div style={{flex: 1}}>
