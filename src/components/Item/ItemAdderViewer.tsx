...
export const ItemAdderViewer: React.FC<Props> = props => {
    ...
    const save = useCallback(
        (value: string) => {
            ...
            const daysFilter = filters
                .find((filter: Filter) => filter.rule.name === 'days'
                    && filter.on
                    && filter.rule.startDay === filter.rule.endDay);

            if (daysFilter) {
                const due: Date = (new Date());
                due.setHours(12, 0, 0, 0);
                due.setDate(due.getDate() + (daysFilter.rule.startDay as number));

                actions.changeItem(childrenId, 'due', undefined, due);
            }
